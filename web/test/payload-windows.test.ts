...
describe('bot windows', () => {
  it('reports each window as a share of the damage it offered', async () => {
  ...
  it('names the most of a window anyone has taken, this run included', async () => {
    const p = await build(GROUND_B);
  ...
  it('uses the same recent set the band is built from', async () => {
    const p = await build(GROUND_B);
    ...
    expect((await build(GROUND_B, { recentN: 0 })).windows[0].recent).toBeNull();
  });

  it('weights the whole-run summary by damage, not by window', async () => {
  ...
  it('reverses cleanly when the baseline is the other run', async () => {
    const p = await build(GROUND_A);
    expect(p.window_summary!.mine).toBe(0.3070336947352315);
    expect(p.window_summary!.base).toBe(0.3358887148407242);
  });

  it('keeps a window with no damage recorded out of the numerator only', async () => {
    ...
    const p = await buildRunPayload(buildStore(rows), GROUND_B);
  ...
  it('is absent on a scenario whose bots actually die', async () => {
