...
aimcurve only ever reads the game's files. The SQLite index deliberately lives
outside the install so nothing aimcurve writes can be mistaken for game data.
"""
...
def default_db_path():
    ...
    return os.path.join(base, "aimcurve", "index.sqlite3")
...
def load(env=None):
    ...
    return Config(
        ...
        stats_dir=stats_dir,
        perf_dir=perf_dir,
        db_path=env.get("AIMCURVE_DB") or default_db_path(),
