...
from aimcurve import paths  # noqa: E402
...
class LoadConfig(unittest.TestCase):
    def test_env_override_points_at_a_fixture_tree(self):
        """Every other aimcurve test module resolves its fixtures through this."""
        ...
        self.assertEqual(
            paths.load({"KOVAAKS_DIR": FIXTURES, "AIMCURVE_DB": "/tmp/x.db"}).db_path,
            "/tmp/x.db",
        )

    def test_missing_root_fails_with_exit_code_4_naming_the_path(self):
        missing = os.path.join(tempfile.gettempdir(), "aimcurve-does-not-exist")
