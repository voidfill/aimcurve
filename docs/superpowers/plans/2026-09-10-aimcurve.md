...
- **No build step.** Frontend assets are committed ready-to-serve. uPlot goes in `aimcurve/web/vendor/` as a committed file. No CDN — it must work offline.
- **Exit codes match `kvpl.py`:** `2` bad input, `4` path/config problem.
- **Never write to the KovaaK's directory.** aimcurve is read-only against the game's files. The SQLite index lives outside it.
...
| File | Responsibility |
|---|---|
| `aimcurve/__init__.py` | Version marker only |
| `aimcurve/__main__.py` | CLI entry: `python -m aimcurve` |
| `aimcurve/paths.py` | Locate KovaaK's dirs, env overrides, `Fail`/exit codes |
| `aimcurve/statscsv.py` | `stats/*.csv` → dict, including cm/360 derivation. Pure |
| `aimcurve/perf.py` | `.perf` → dense per-second arrays. Pure |
| `aimcurve/index.py` | SQLite schema, bootstrap, incremental indexing, failure tracking |
| `aimcurve/watch.py` | Two-tier poll loop, new-run callback |
| `aimcurve/compare.py` | Baseline selection, smoothing, alignment, cumulative delta. Pure |
| `aimcurve/server.py` | HTTP handler, JSON endpoints, SSE stream |
| `aimcurve/web/index.html` | Dashboard shell |
| `aimcurve/web/app.js` | Fetch, render, SSE subscribe, controls |
| `aimcurve/web/style.css` | Layout |
| `aimcurve/web/vendor/uPlot.iife.min.js`, `uPlot.min.css` | Vendored chart lib |
| `tests/fixtures/` | Real CSV/`.perf` pairs + synthesised edge cases |
| `tests/test_*.py` | One test module per component (see the testing policy below) |
...
**Files:**
- Create: `aimcurve/__init__.py`, `aimcurve/paths.py`
- Create: `tests/fixtures/` (populated from the real install)
...
```bash
mkdir -p tests/fixtures/stats tests/fixtures/performances
...
dst = os.path.join("tests", "fixtures", "stats")
...
python - <<'EOF'
import os
src = os.path.join("tests","fixtures","aimcurve","performances",
                   "Pasu Voltaic Reload Easier - Challenge - 2026.07.27-16.50.01 Performance.perf")
raw = open(src, "rb").read()
dst = os.path.join("tests","fixtures","aimcurve",
...
Expected: FAIL — `ModuleNotFoundError: No module named 'aimcurve'`

- [ ] **Step 4: Implement `aimcurve/paths.py`**
...
Also create `aimcurve/__init__.py`:
...
```bash
git add aimcurve/__init__.py aimcurve/paths.py tests/test_paths.py \
        tests/fixtures .gitignore
git commit -m "Add aimcurve package skeleton, path resolution, and real-run fixtures"
```
...
- [ ] **Step 3: Implement `aimcurve/statscsv.py`**
...
```bash
git add aimcurve/statscsv.py tests/test_statscsv.py
...
- [ ] **Step 3: Implement `aimcurve/perf.py`**
...
```bash
git add aimcurve/perf.py tests/test_perf.py
...
- [ ] **Step 3: Implement `aimcurve/index.py`**
...
from aimcurve import index, paths
cfg = paths.load({"AIMCURVE_DB": os.path.join(tempfile.mkdtemp(), "i.sqlite3")})
...
```bash
git add aimcurve/index.py tests/test_index.py
...
Run: `python -m unittest tests.test_watch -v`
Expected: FAIL — `ImportError: cannot import name 'watch'`

- [ ] **Step 3: Implement `aimcurve/watch.py`**
...
```bash
git add aimcurve/watch.py tests/test_watch.py
...
- [ ] **Step 3: Implement `aimcurve/compare.py`**
...
**Files:**
- Create: `aimcurve/server.py`, `aimcurve/__main__.py`
- Create: `aimcurve/web/index.html` (one-line placeholder; Task 8 replaces it)
...
|---|---|
| `GET /` | `web/index.html` |
| `GET /web/*` | static assets from `aimcurve/web/` |
...
- [ ] **Step 3: Implement `aimcurve/server.py`**
...
```bash
mkdir -p aimcurve/web/vendor
printf '<!-- aimcurve -->\n<h1>aimcurve</h1>\n' > aimcurve/web/index.html
```

- [ ] **Step 5: Commit**

```bash
git add aimcurve/server.py aimcurve/__main__.py aimcurve/web/index.html \
...
**Files:**
- Create: `aimcurve/web/style.css`, `aimcurve/web/app.js`
- Modify: `aimcurve/web/index.html` (replace the placeholder)
- Create: `aimcurve/web/vendor/uPlot.iife.min.js`, `aimcurve/web/vendor/uPlot.min.css`
...
curl -fsSL https://cdn.jsdelivr.net/npm/uplot@1.6.31/dist/uPlot.iife.min.js \
  -o aimcurve/web/vendor/uPlot.iife.min.js
curl -fsSL https://cdn.jsdelivr.net/npm/uplot@1.6.31/dist/uPlot.min.css \
  -o aimcurve/web/vendor/uPlot.min.css
ls -l aimcurve/web/vendor/
```
...
- [ ] **Step 2: Write `aimcurve/web/index.html`**
...
- [ ] **Step 3: Write `aimcurve/web/style.css`**
...
- [ ] **Step 4: Write `aimcurve/web/app.js`**
...
- **`aimcurve`** — a local dashboard over your run history. Read-only; it never writes
  to the game.

Game settings and benchmark rank tracking remain out of scope for both.
```

- [ ] **Step 2: Document aimcurve**
...
```
aimcurve/                 the dashboard: parser, index, watcher, server, web assets
```
...
```
tests/                   unit tests for the playlist CLI and every aimcurve module
```

- [ ] **Step 4: Run the whole suite**

Run: `python -m unittest discover -s tests -v`
Expected: PASS, 34 tests — the 9 existing `kvpl` tests plus 25 `aimcurve` ones.

- [ ] **Step 5: Commit**

```bash
git add README.md
git commit -m "Document aimcurve and correct the repo scope statement"
```
