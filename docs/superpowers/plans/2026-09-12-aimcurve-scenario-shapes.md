# aimcurve Scenario Shapes Implementation Plan
...
**Architecture:** A new pure module `aimcurve/shapes.py` decides `timed` vs `race` from evidence the index already holds. `index.py` gains a `kill` table and a `scenario` table at schema v2. `compare.py` resamples race curves onto a cumulative-damage progress grid; its delta math is unchanged, only its index changes. `server.py` emits a shape-tagged payload and `app.js` renders whichever axis the payload names.

**Tech Stack:** Python 3 stdlib only (no packages, no build step). `unittest`, run with `python -m unittest discover -s tests`. Frontend is plain ES2020 plus the vendored uPlot already in `aimcurve/web/vendor/`.

**Spec:** `docs/superpowers/specs/2026-09-12-aimcurve-scenario-shapes-design.md`

## Global Constraints

- **Stdlib only.** No new dependencies, in the package or the tests.
- **Never write to the KovaaK's install.** aimcurve reads game files and writes only its own SQLite cache.
...
- **`SCHEMA_VERSION` becomes `2`** in this plan (currently `1`, `aimcurve/index.py:16`).
...
- **Test style:** `unittest.TestCase`, `sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))` then `from aimcurve import ...  # noqa: E402`. Match the existing files.
...
**Files:**
- Modify: `aimcurve/statscsv.py` (add `parse_kills`, extend `parse`)
- Test: `tests/test_statscsv.py`
- Add fixtures: `tests/fixtures/stats/` and `tests/fixtures/performances/`
...
Append to `tests/test_statscsv.py`:
...
Run: `python -m unittest tests.test_statscsv -v`
Expected: FAIL with `AttributeError: module 'aimcurve.statscsv' has no attribute 'parse_kills'`

- [ ] **Step 4: Implement**

In `aimcurve/statscsv.py`, add to the `_INTS` map:
...
```bash
git add aimcurve/statscsv.py tests/test_statscsv.py tests/fixtures
...
from aimcurve import shapes  # noqa: E402
...
Run: `python -m unittest tests.test_shapes -v`
Expected: FAIL with `ModuleNotFoundError: No module named 'aimcurve.shapes'`
...
```bash
git add aimcurve/shapes.py tests/test_shapes.py
...
**Files:**
- Modify: `aimcurve/index.py` (`SCHEMA_VERSION`, `SCHEMA`, `_RUN_COLUMNS`, `index_stats_file`, `attach_perf`, `bootstrap`)
...
class ScenarioShapes(unittest.TestCase):
    def setUp(self):
        ...
        self.cfg = paths.load({"KOVAAKS_DIR": self.dir,
                               "AIMCURVE_DB": os.path.join(self.dir, "i.sqlite3")})
...
`tests/test_index.py` already imports `os`, `shutil`, `sys`, `tempfile`, `unittest` and `from aimcurve import index, paths` and defines `FIXTURES`. Reuse them; do not re-import.

- [ ] **Step 2: Run the test to verify it fails**

Run: `python -m unittest tests.test_index -v`
Expected: FAIL with `AttributeError: module 'aimcurve.index' has no attribute 'scenario_row'`

- [ ] **Step 3: Bump the schema and add the tables**

In `aimcurve/index.py`, set `SCHEMA_VERSION = 2` and add to the `SCHEMA` string, after the `curve` table:
...
Two places in `aimcurve/watch.py` change a scenario's evidence, and both must refresh it — indexing a run adds a `(score, elapsed)` pair for tier 2, and attaching its `.perf` adds the curve that tier 1 needs. A live run that only refreshed on one of them would sit on the wrong shape until the next restart.
...
Run: `python -m unittest tests.test_index tests.test_watch -v`
Expected: PASS
...
```bash
git add aimcurve/index.py aimcurve/watch.py tests/test_index.py
...
**Files:**
- Modify: `aimcurve/compare.py` (`candidates`, plus new resampling functions)
...
class RaceBaselines(unittest.TestCase):
    def test_the_duration_filter_is_skipped_for_race_scenarios(self):
        ...
        cfg = paths.load({"KOVAAKS_DIR": directory,
                          "AIMCURVE_DB": os.path.join(directory, "i.sqlite3")})
...
Expected: FAIL with `AttributeError: module 'aimcurve.compare' has no attribute 'race_grid'`

- [ ] **Step 3: Implement the resampling**

In `aimcurve/compare.py`, add `from . import shapes` to the imports and append:
...
git add aimcurve/compare.py tests/test_compare.py
...
**Files:**
- Modify: `aimcurve/server.py` (`build_run_payload`, `/api/runs`, `/api/scenarios`)
- Test: `tests/test_server.py` (rewrite the payload assertions)
...
Replace the body of `class RunPayload` in `tests/test_server.py` with:
...
In `aimcurve/server.py`, add `from . import shapes` and replace `build_run_payload` with:
...
```bash
python -m unittest discover -s tests
git add aimcurve/server.py tests/test_server.py
...
**Files:**
- Modify: `aimcurve/web/app.js`, `aimcurve/web/style.css`, `aimcurve/web/index.html`

**Interfaces:**
- Consumes: the Task 5 payload.
- Produces: no exported interface — this is the last consumer.

Read `aimcurve/web/app.js` in full before editing. The published design preview at the URL in the conversation shows the intended result; match its behaviour, not its styling (the app has its own tokens already).
...
```bash
git add aimcurve/web/app.js aimcurve/web/style.css aimcurve/web/index.html
...
**Files:**
- Create: `tests/test_shapes_integration.py`
- Modify: `README.md`
...
Run: `python -m unittest tests.test_shapes_integration -v`
...
In `README.md`, in the `aimcurve` section, add a third bullet to the "Two things worth knowing about the data" list — and change that lead-in to "Three things worth knowing about the data":
...
```bash
git add tests/test_shapes_integration.py README.md
...
Once Task 7 is reviewed, use `superpowers:finishing-a-development-branch` to decide how `aimcurve-scenario-shapes` gets integrated.
