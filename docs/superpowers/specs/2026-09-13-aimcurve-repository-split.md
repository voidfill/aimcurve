# Splitting aimcurve into its own repository
...
`kovaaks-playlists` and `aimcurve` share no code. No import crosses between them
...
clone a repository and run an agent. aimcurve is for KovaaK's players who want to
...
Everything aimcurve: the `aimcurve/` package, `tests/test_*.py`,
`tests/fixtures/`, `scripts/drive-client.mjs`, and the aimcurve specs —
including the three browser documents.

**The fixtures go with aimcurve, and so does the frozen Python.** An earlier
...
Python is the oracle, the JS is the product, and `tests/fixtures/` is
...
more than anything else in this document. `aimcurve` is taken as a concept by
