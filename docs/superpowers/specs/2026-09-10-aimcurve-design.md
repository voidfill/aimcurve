# KovaaK's Stats Dashboard (`aimcurve`) — Design
...
scope for this repo. That line is superseded: `aimcurve` is a sibling to the playlist
...
```
aimcurve/
  __main__.py   python -m aimcurve  ->  serve
...
7. README: replace the out-of-scope line, document `python -m aimcurve`.
